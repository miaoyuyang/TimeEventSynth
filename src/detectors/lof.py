"""Local Outlier Factor detector backbone wrapper."""
...
# joblib/loky warns when physical core count cannot be probed (common in CI/sandbox).
# A user cap strictly below the logical core count skips the broken physical-core probe.
if not str(os.environ.get("LOKY_MAX_CPU_COUNT", "")).strip():
...
@dataclass
class LOFDetector(DetectorBackbone):
    window_size: int = 15
    stride: int = 1

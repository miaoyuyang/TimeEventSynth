"""One-Class SVM detector backbone wrapper."""
...
@dataclass
class OCSVMDetector(DetectorBackbone):
    window_size: int = 15
    stride: int = 1
    kernel: str = "rbf"
    ...
    def score(self, test_series: list[Any], config: dict[str, Any] | None = None) -> dict[str, np.ndarray]:
        ...
        for record in test_series:
            ...
            window_scores = -self.model.decision_function(features)
